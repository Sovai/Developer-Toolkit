...
class NaturalTTS {
  ...
  async generateAudio() {
    ...
    const maxChunkSize = 4000; // OpenAI TTS limit is around 4096 characters
    if (this.currentText.length > maxChunkSize) {
      return await this.generateChunkedAudio();
    }

    this.showLoading(true);
    this.showStatus('Generating audio with OpenAI TTS...', 'info');
  ...
  async generateChunkedAudio() {
    ...
    this.audioQueue = [];
    this.currentChunkIndex = 0;
    this.chunks = this.splitTextIntoChunks(this.currentText, 4000);
  ...
  async generateSingleChunk(chunkIndex) {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(`Chunk ${chunkIndex + 1}: ${error.error?.message || 'Failed to generate audio'}`);
      }
